module.exports.index =async (req,res)=>{
    ...
    res.send((await frameWork.productServices.getAllProducts()));
...
module.exports.add = async(req=req,res)=>{
    try{
        ...
        if(await(frameWork.productServices.addProduct(product))){
            res.status(200).send("product added");
        }else{
            throw Error("request body error ")
        }
    }catch(e){
        res.status(400).send("unable to add product",e.message);
        console.log(e);
    }
...
module.exports.update = async(req,res)=>{
    try{
        ...
        if(await(frameWork.productServices.updateProduct(id,update))){
...
module.exports.getProduct = async(req,res)=>{
    ...
    res.status(200).send(await(frameWork.productServices.getProductById(req.params.id)));
